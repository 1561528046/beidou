        ...
        <el-row :gutter="20">
          ...
          <el-col style="text-align:center">
            <el-form-item label-width="0">
              <el-button @click="updateLinkage" type="primary">提交</el-button>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    //查询单个报警联动信息
    getLinkage() {
      getAlarmLinkByID({ alarm_link_id: this.$props.linkage_id }).then(res => {
        if (res.data.code == 0) {
          this.$set(this.$data, "formData", res.data.data[0]);
          this.formData.alarm_type.split(",").map(itam => {
            this.$refs.multipleTable.data.map((item, index) => {
              if (item.alarm_type == itam) {
                this.alarm.push(item.alarm_type);
          ...
          this.formData.logical_channel.split(",").map(itbm => {
            this.channel.push(itbm);
          });
          this.$set(this.formData, "time", [
            res.data.data[0].StartTime,
            res.data.data[0].EndTime
          ]);
        } else {
          return this.$notify({
            message: res.data.msg,
    ...
    updateLinkage() {
      ...
      updateAlarmLink(this.formData).then(res => {
        if (res.data.code == 0) {
          this.$emit("success");
          return this.$notify({
            message: "修改成功",
