<template>
  <div class="shadow-box monitor-info" :class="{'small-size':smallSize}">
    <div class="_close">
      <i
        class="iconfont icon-window-minimize window-ctrl"
        v-if="!smallSize"
        @click="smallSize = true"
        style="position:relative;top:-7px;"
      ></i>
      <i
        class="iconfont icon-window-maximize window-ctrl"
        v-if="smallSize"
        @click="smallSize = false"
      ></i>
    </div>
    <el-tabs v-model="currentTab" type="border-card" @tab-click="smallSize = false">
      ...
      <el-tab-pane :closable="false" name="list809-check">
        <span slot="label">查岗
          <el-badge
            :value="$store.getters['x809In/list9301'].length"
            :max="99"
            class="monitor-badge"
          ></el-badge>
        </span>
      ...
      <el-tab-pane :closable="false" name="list809-notice">
        <span slot="label">报文
          <el-badge
            :value="$store.getters['x809In/list9302'].length"
            :max="99"
            class="monitor-badge"
          ></el-badge>
        </span>
